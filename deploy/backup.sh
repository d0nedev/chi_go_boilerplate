...
#   15 2 * * * /path/to/repo/deploy/backup.sh >> /var/log/chi-product-backup.log 2>&1
# Set BACKUP_REMOTE (an rclone remote, e.g. "r2:chi-product-backups") to also copy
...
name="backups/chi_product-$(date -u +%Y%m%dT%H%M%SZ).dump"
